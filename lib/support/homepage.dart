...
// //                 const SizedBox(height: 25),
// //                 // ສ່ວນສະຫຼຸບການເງິນ
// //                 _buildFinanceSummaryRow(context, totalInc, totalExp),
// //                 const SizedBox(height: 25),
// //               ],
// //             );
// //           },
...
// //             onTap: () {
// //               // 2. สั่งให้เปิดหน้า AddIncomePage
// //               Navigator.push(
// //                 (context),
...
//             // ສ້າງ List ສຳລັບເກັບທຸລະກຳທັງຫມົດເພື່ອສະແດງ Recent Transactions
//             List<DocumentSnapshot> allTransactions = [];

//             if (expenseSnapshot.hasData) {
//               allTransactions.addAll(expenseSnapshot.data!.docs);
//               for (var doc in expenseSnapshot.data!.docs) {
//                 double amt = (doc['amount'] ?? 0).toDouble();
//                 String cat = doc['category'] ?? "Food";
//                 totalExp += amt;
...
//             if (incomeSnapshot.hasData) {
//               allTransactions.addAll(incomeSnapshot.data!.docs);
//               for (var doc in incomeSnapshot.data!.docs) {
//                 totalInc += (doc['amount'] ?? 0).toDouble();
//               }
//             }

//             // ລຽງລຳດັບ ທຸລະກຳຂອງເຮົາໄຫ້ມັນເປັນແຖ້ວ ເມື່ອມີການເພີມຂໍ້ມູນຄົ້ງລາສຸດ ໄຫ້ມັນສະແດງຢູ່ເທິງສຸດທັນທີ
//             allTransactions.sort((a, b) {
//               Timestamp dateA = a['createdAt'] ?? Timestamp.now();
//               Timestamp dateB = b['createdAt'] ?? Timestamp.now();
...
//                 // --- ເພີມສວນ Recent Transactions ບອນນີ---
//                 Row(
//                   mainAxisAlignment: MainAxisAlignment.spaceBetween,
//                   children: [
//                     const Text(
...
//                       ? 5
//                       : allTransactions.length, // ສະແດງ 5 ລາຍການລາສຸດ
//                   itemBuilder: (context, index) {
//                     var doc = allTransactions[index];
//                     var data = doc.data() as Map<String, dynamic>;
//                     bool isExpense = doc.reference.parent.id == "expenses";
...
//                             children: [
//                               Text(
//                                 "${isExpense ? 'Expense' : 'Income'}  $dateStr",
//                                 style: const TextStyle(
              ...
              children: [
                ...
                ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    // --- แก้ไขตรงนี้ เพื่อให้โชว์หมวดหมู่ที่เลือก ---
                    String displayName = isExpense
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        color: Colors
                            .grey[100], // ปรับสีให้อ่อนลงเพื่อให้ตัวหนังสือชัดขึ้น
                        borderRadius: BorderRadius.circular(15),
                      ),
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Column(
                            ...
                            children: [
                              Text(
                                "$displayName", // แสดงชื่อหมวดหมู่ (Car, Food, etc.)
                                style: const TextStyle(
                                  fontSize: 16,
                                  fontWeight: FontWeight.bold,
                                ),
